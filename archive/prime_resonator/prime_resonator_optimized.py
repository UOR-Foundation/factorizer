# prime_resonator.py
"""
Prime Resonator v3.1 (Tuned for 96-bit) – **Enhanced multi‑view hierarchical resonance**
======================================================================
...
All demos print:
```
Prime Resonator v3.1 (Tuned for 96-bit) | 2025‑06‑08T16:05:12 | factors found in 0.014360 s
n = p × q
```
"""
...
def _adaptive_prime_count(n: int) -> int:
    ...
    if bit_len < 64:
        return 32  # Increased from 24 based on tuning
    elif bit_len < 96:
        return min(64, 32 + bit_len // 3)  # Faster growth for medium (was // 4)
    elif bit_len < 128:
...
def _golden_positions(n: int) -> set[int]:
    ...
    bit_len = n.bit_length()
    num_samples = min(50 if bit_len < 80 else 75 if bit_len < 96 else 40, int(math.log2(n)))
...
def _coordinate_convergence(n: int, coords: List[int]) -> set[int]:
    ...
    max_crt_pairs = 5 if bit_len < 80 else 7 if bit_len < 96 else 4
...
def _multiplicative_resonance_score(n: int, cand: int, coords: List[int]) -> float:
    ...
    for i in range(check_primes):
        ...
        if (cand % p) == c:
            harmonic *= (1 + 1.5 / p)  # Stronger resonance bonus
        else:
...
_ALGO_NAME = "Prime Resonator v3.1 (Tuned for 96-bit)"
...
def prime_resonate(n: int) -> Tuple[int, int]:
    ...
    for c in filtered_cands[:max_phase1_candidates]:
        ...
        if score > (0.5 if bit_len < 96 else 0.4):  # Lower threshold for larger numbers
    ...
    top_count = min(100 if bit_len < 80 else 150 if bit_len < 96 else 75, len(scored))

...
use crate::types::{Observation, Pattern, PatternKind, ScaleRange};
use crate::Result;
...
impl InvariantDiscovery {
        ...
        observations.iter().all(|obs| {
            ...
            obs.modular.modular_signature.iter().enumerate().all(|(i, &n_mod)| {
                if i < obs.modular.p_mod_signature.len() && i < obs.modular.q_mod_signature.len() {
                    ...
                    let prime = crate::utils::generate_primes(i + 1)[i].as_integer().to_u64().unwrap();
    ...
    fn discover_emergent_invariants(observations: &[Observation]) -> Result<Vec<Pattern>> {
        ...
        let sqrt_fermat_relation = observations.iter().all(|obs| {
            obs.derived.sqrt_n < obs.derived.fermat_a
        });
        ...
        let positive_offset = observations.iter()
            .filter(|obs| obs.scale.pattern_type == crate::types::observation::PatternClass::Balanced)
        ...
        if positive_offset {
            ...
            pattern.frequency = 1.0;
            pattern.description = "Offset from sqrt(n) is always positive for balanced semiprimes".to_string();
            patterns.push(pattern);
        }

...
use crate::types::{DecodingStrategy, Factors, Formalization, Number, Pattern};
use crate::utils;
use crate::Result;
        ...
        for offset in 0..=search_radius {
            for sign_p in &[-1i64, 1] {
                for sign_q in &[-1i64, 1] {
                    let p_candidate = Number::from((p as i64 + sign_p * offset as i64).abs() as u64);
                    let q_candidate = Number::from((q as i64 + sign_q * offset as i64).abs() as u64);
                    
                    if &p_candidate * &q_candidate == *n &&
                       p_candidate > Number::from(1u32) &&
                       q_candidate > Number::from(1u32) {
                        return Ok(Factors::new(p_candidate, q_candidate, "quantum_materialization"));
                    }
                }
            }
        }

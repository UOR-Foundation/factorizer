...
use crate::types::{
    DecodingStrategy, Formalization, Pattern, PatternMatrix, PatternType, Recognition,
    UniversalConstant,
};
use crate::Result;
use ndarray::Array2;
...
fn build_universal_encoding(
    ...
    // Product phase
    encoding.insert("product_phase".to_string(), (phi * pi) % (2.0 * std::f64::consts::PI));
    ...
    encoding.insert("unity_coupling".to_string(), unity / (2.0 * std::f64::consts::PI));
    ...
    for constant in constants {
        if constant.universality > 0.5 {
            let value = recognition.signature.get_component(&format!("{}_component", constant.name))
                .unwrap_or(0.0);
            encoding.insert(format!("{}_encoding", constant.name), value * constant.value);
        }
    }
...
fn compute_harmonic_series(
    ...
    for k in 1..=20 {
        let harmonic = base_freq.powi(k as i32) + 
                       recognition.signature.get_component("pi_component").unwrap_or(0.0).powi(k as i32) +

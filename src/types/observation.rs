...
impl Observation {
    ...
    pub fn matches_pattern(&self, other: &Observation) -> f64 {
        ...
        for (a, b) in self.modular.modular_signature.iter().zip(&other.modular.modular_signature) {
            if a == b {
                similarity += 1.0;
            }
            count += 1.0;

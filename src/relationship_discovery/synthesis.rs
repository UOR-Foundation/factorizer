...
impl RelationshipSynthesis {
    ...
    fn discover_meta_patterns(patterns: &[Pattern]) -> Result<Vec<MetaPattern>> {
        ...
        if let (Some(invariants), Some(universals)) = 
            ...
            if invariants.len() >= 3 && universals.len() >= 2 {
                meta_patterns.push(MetaPattern {
                    ...
                    description: "Invariants connect to universal patterns through scale".to_string(),
                    components: invariants.iter()
                        .take(3)
                        .map(|p| p.id.clone())
        ...
        let hierarchical_patterns: Vec<&Pattern> = patterns.iter()
            .filter(|p| p.scale_range.unbounded || 
                       (p.scale_range.max_bits - p.scale_range.min_bits > 100))
            .collect();
        ...
        if hierarchical_patterns.len() >= 3 {
            meta_patterns.push(MetaPattern {
                ...
                components: hierarchical_patterns.iter()
                    .take(5)
                    .map(|p| p.id.clone())
                    .collect(),
                strength: 0.7,
        ...
        let balanced_patterns: Vec<&Pattern> = patterns.iter()
            .filter(|p| p.id.contains("balanced"))
            .collect();
        let harmonic_patterns: Vec<&Pattern> = patterns.iter()
            .filter(|p| p.id.contains("harmonic"))
            .collect();
    ...
    fn discover_emergent_laws(
        ...
        if offset_ratios.len() >= 10 {
            let max_ratio = offset_ratios.iter().fold(0.0, |a, &b| a.max(b));
        ...
        if scale_gaps.len() >= 3 {
            ...
            let growth_observed = scale_gaps.windows(2)
                .all(|w| w[1].1 >= w[0].1);
    ...
    fn build_unified_theory(
        ...
        if !constants.is_empty() {
            let constant_names: Vec<String> = constants.iter()
                .map(|c| c.name.clone())
                .collect();
        ...
        let scale_invariant_patterns: Vec<String> = patterns.iter()
            .filter(|p| p.scale_range.unbounded)
            .map(|p| p.id.clone())
            .collect();
    ...
    pub fn summarize(&self) -> String {
        format!(
            ...
            self.meta_patterns.iter()
                .max_by(|a, b| a.strength.partial_cmp(&b.strength).unwrap())
                .map(|p| &p.id)
                .unwrap_or(&"none".to_string()),
            self.emergent_laws.iter()
                .max_by(|a, b| a.confidence.partial_cmp(&b.confidence).unwrap())
                .map(|l| &l.name)
                .unwrap_or(&"none".to_string()),
            self.unified_components.iter()

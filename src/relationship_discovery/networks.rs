...
impl PatternNetwork {
    ...
    pub fn build(
        ...
        for (edge_idx, corr) in correlations.iter().enumerate() {
            if corr.p_value < 0.05 && corr.correlation.abs() > 0.3 {
                let edge_type = if corr.correlation > 0.0 {
                    EdgeType::Positive
                } else {
                    EdgeType::Negative
                };
    ...
    pub fn get_community_members(&self, pattern_id: &str) -> Vec<&PatternNode> {
        let community = self.nodes.iter()
            .find(|n| n.id == pattern_id)
            .and_then(|n| n.community);
        
        if let Some(comm_id) = community {
            self.nodes.iter()
                .filter(|n| n.community == Some(comm_id))
                .collect()
        } else {
            vec![]
        }
    ...
    pub fn get_statistics(&self) -> NetworkStatistics {
        ...
        let community_count = self.nodes.iter()
            .filter_map(|n| n.community)
            .max()
            .unwrap_or(0) + 1;
        
        let positive_edges = self.edges.iter()
            .filter(|e| e.edge_type == EdgeType::Positive)
            .count();
        
        let negative_edges = self.edges.iter()
            .filter(|e| e.edge_type == EdgeType::Negative)
            .count();

...
use rust_pattern_solver::pattern::{recognition, formalization, execution};
...
fn main() -> rust_pattern_solver::Result<()> {
    ...
    for (n, factors) in test_cases {
        ...
        match recognition::recognize(n.clone(), &patterns) {
            Ok(recognition) => {
                ...
                let max_resonance = recognition.signature.resonance.iter()
                    .copied()
                    .fold(f64::NEG_INFINITY, f64::max);
                println!("  Resonance field: {} points, max: {:.3}", field_len, max_resonance);
                
                // Continue to formalization
                match formalization::formalize(recognition, &patterns, &[]) {
                    Ok(formalization) => {
                        println!("  Formalized with {} strategies", formalization.strategies.len());
    ...
    println!("  Recognized {}/{} numbers in {:.2}s ({:.2} per second)",

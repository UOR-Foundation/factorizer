...
fn main() -> rust_pattern_solver::Result<()> {
    ...
    for pattern in &emergent.invariants {
        ...
        if !pattern.scale_range.unbounded {
            println!(
                "    Scale: {}-{} bits",
                pattern.scale_range.min_bits, pattern.scale_range.max_bits
            );
        }
    }
    ...
    println!(
        "\nUniversal Patterns ({}):",
        emergent.universal_patterns.len()
    );
    ...
    let high_freq_patterns =
        emergent.all_patterns().into_iter().filter(|p| p.frequency >= 0.8).count();
    println!("High-frequency patterns (≥80%): {}", high_freq_patterns);

    // Check for specific interesting patterns
    let has_phi = emergent.universal_patterns.iter().any(|p| p.id.contains("phi"));
    let has_pi = emergent.universal_patterns.iter().any(|p| p.id.contains("pi"));
    let has_e = emergent
        .universal_patterns
        .iter()
    ...
    for &scale in scales.iter().take(5) {
        if let Some(obs_at_scale) = by_scale.get(&scale) {
            let balanced_ratio =
                obs_at_scale.iter().filter(|obs| obs.scale.balance_ratio < 1.1).count() as f64
                    / obs_at_scale.len() as f64;

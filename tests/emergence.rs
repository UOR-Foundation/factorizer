...
fn test_invariant_discovery() {
    ...
    let has_multiplication = invariants.iter()
        .any(|p| p.id == "multiplication_invariant");
    let has_fermat = invariants.iter()
        .any(|p| p.id == "fermat_identity");
    let has_modular = invariants.iter()
        .any(|p| p.id == "modular_invariant");
    
    assert!(has_multiplication, "Failed to discover multiplication invariant");
...
fn test_scaling_analysis() {
    ...
    assert!(!scaling_patterns.is_empty(), "No scaling patterns discovered");
    
    // Should discover some scale-dependent patterns
    let scale_dependent_count = scaling_patterns.iter()
        .filter(|p| matches!(p.kind, PatternKind::ScaleDependent))
        .count();
    
    assert!(scale_dependent_count > 0, "No scale-dependent patterns found");
}
...
fn test_universal_pattern_discovery() {
    ...
    assert!(!universal_patterns.is_empty(), "No universal patterns discovered");
    ...
    for pattern in &universal_patterns {
        println!("Universal pattern: {} - {}", pattern.id, pattern.description);
    }
}
...
fn test_pattern_emergence_without_bias() {
    ...
    println!("Discovered {} total patterns from random semiprimes", emergent.count());

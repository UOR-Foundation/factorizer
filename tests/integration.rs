...
fn test_full_pipeline_small_numbers() {
    ...
    let training_numbers: Vec<Number> = vec![15, 21, 35, 77, 91, 143, 221, 323, 437, 667, 899]
        .into_iter()
        .map(Number::from)
        .collect();
...
fn test_batch_processing() {
    ...
    println!(
        "Observed {} numbers in {:.2}s ({:.0} per second)",
        numbers.len(),

...
fn test_small_semiprimes() {
    ...
    let numbers: Vec<Number> = test_cases.iter()
    ...
    for (n, expected_p, expected_q) in test_cases {
        ...
        assert_eq!(p, Number::from(expected_p), "Wrong smaller factor for {}", n);
...
fn test_perfect_squares() {
    ...
    let mut collector = ObservationCollector::new();
    let numbers: Vec<Number> = squares.iter()
        .map(|(n, _)| Number::from(*n))
        .collect();
...
fn test_small_factor_cases() {
    ...
    let mut collector = ObservationCollector::new();
    let numbers: Vec<Number> = cases.iter()
        .map(|(n, _, _)| Number::from(*n))
        .collect();
